...
/*
function minMax(arrMM = []){
let min = arrMM[0];
let max = 1;
    for (let i = 0; i<arrMM.length; i++){
        if (arrMM[i] > arrMM[i+1] && arrMM[i] > max ){
            max = arrMM[i];
        }else if (arrMM[i] < arrMM[i+1] && arrMM[i+1] <= min){
            min = arrMM[i];
        }
    }
    console.log(max);
...
minMax([4,56,89,56,23,10,456,8,20,14]);
